...
from utils import to_byte, to_bchr, to_bytestr
...
class MateTester(MateNET):
    ...
    def send_packet(self, payload):
        ...
        """
        data = to_bytestr(0x03) + to_bytestr(payload)  # TODO: not sure why 0x03 here
        self._send(data)
    ...
    def recv_packet(self, timeout=1.0):
        ...
        port = to_byte(data[0])   # I think this is the port the device is connected to?
        ptype = to_byte(data[1])  # Type of the packet
        payload = [to_byte(c) for c in data[2:]]
...
class MXEmulator(MateTester):
    ...
    def packet_status(self, packet):
        ...
        self.send_packet(
            ...
            + b'\x80'  # In current
            + b'\x82'  # Out current
            + b'\x00'  # kWH (signed, upper)
            + b'\x00'  # Ah (lower)
            + b'\x3F'
            + b'\x02\x01'  # Status/Error
            + b'\xF0'  # kWH (signed, lower)
            + b'\x03\xE7'  # Bat voltage
            + b'\x27\x0F'  # PV voltage
        )
    ...
    def packet_log(self, packet):
        ...
        self.send_packet(b'\x02\xFF\x17\x01\x16\x3C\x00\x01\x01\x40\x00\x10\x10' + to_bytestr(day))
        #self.send_packet(b'\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF' + to_bytestr(day))
        #self.send_packet(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x48' + to_bytestr(day))
...
class FXEmulator(MateTester):
    ...
    def packet_status(self, packet):
        ...
        self.send_packet(
            b'\x02\x28\x0A'
            + b'\x02\x01'
            + b'\x0A\x00\x64\x00\x00\xDC\x14\x0A'
        )
...
class FlexNETDCEmulator(MateTester):
    ...
    def packet_status(self, packet):
        ...
        self.send_packet(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00')
